...
// Índice de columna aceptando sinónimos
...
// Normalización de fecha (DD/MM/YYYY)
...
// Normalización de hora a HH:mm (acepta 24h o 12h am/pm)
...
/* ===== Mapeo dinámico por encabezado =====
   Acepta sinónimos de fecha/hora:
   - fecha:  fecha_cita, fecha_dia, fecha
   - hora:   hora_cita, hora_cta, hora
*/
function valueByHeader(data, numeroCitaGenerado, observacionFinal) {
  const fechaNorm = normalizarFecha(data.fecha_cita || "");
  const horaNorm = normalizarHora(data.hora_cita || "");
  return {
...
export async function updateAppointmentStatusByCedula(cedula, nuevoEstado) {
  try {
    ...
    const cedulaIdx = getColIndex(normalized, ["numero_cedula", "cedula", "documento"]);
    const statusIdx = getColIndex(normalized, ["status_cita", "estado", "status"]);
...
/* ===== Disponibilidad de hora (con normalización y sinónimos) ===== */
export async function isHoraDisponible(fecha, hora) {
  try {
    ...
    const fechaIdx = getColIndex(normalized, ["fecha_cita", "fecha_dia", "fecha"]);
    const horaIdx = getColIndex(normalized, ["hora_cita", "hora_cta", "hora"]);

    if (fechaIdx === -1 || horaIdx === -1) {
      console.warn("⚠️ Columnas de fecha/hora no encontradas. Bloqueando para evitar duplicados.");
      return false;
    }
    ...
    // En caso de error, bloquear para evitar duplicados
    return false;
  }
}
...
export async function getHorasDisponibles(fecha) {
  try {
    ...
    // Horario base 08:00 a 17:00 cada 60 min
    const horarioBase = [];
