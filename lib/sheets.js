...
/**
 * Actualiza una cita existente identificada por cédula.
 * Permite cambiar fecha, hora y otros campos.
 * @param {string} cedula - Número de cédula para buscar la cita.
...
export async function updateAppointmentByCedula(cedula, nuevosDatos) {
  try {
    ...
    // Actualizar solo las columnas que existan en la hoja y en nuevosDatos
    for (const [campo, valor] of Object.entries(nuevosDatos)) {
      const colIdx = getColIndex(normalized, [campo]);
      if (colIdx !== -1 && valor !== undefined && valor !== null && valor !== "") {
        ...
        await sheets.spreadsheets.values.update({
          ...
          range,
          valueInputOption: "USER_ENTERED",
          requestBody: { values: [[campo.includes("fecha") ? normalizarFecha(valor) : campo.includes("hora") ? normalizarHora(valor) : valor]] }
        });
      }
    }
