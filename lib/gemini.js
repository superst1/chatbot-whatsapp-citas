...
export async function procesarCitaConGemini(estadoActual, mensajeUsuario) {
  const prompt = `
Eres un asistente para agendar, re-agendar y cancelar citas médicas.

Estado actual de la cita: ${JSON.stringify(estadoActual || {})}
Mensaje nuevo del usuario: "${mensajeUsuario}"

Debes devolver SOLO un JSON válido, sin texto adicional, sin backticks, sin \`\`\`json, con esta estructura:
{
  "datos": {
    "nombre_paciente": "...",
    "numero_cedula": "...",
    ...
    "nombre_contacto": "...",
    "celular_contacto": "...",
    "accion": "crear|reagendar|cancelar"
  },
  "completo": true/false,
...
- Si detectas re-agenda, pon "accion": "reagendar" y actualiza fecha/hora.
- Si detectas cancelación, pon "accion": "cancelar".
- Si falta algo, en "respuesta" pide solo lo que falta.
- NO incluyas \`\`\` ni texto fuera del JSON.
`;

  const raw = await handleMessage(prompt);

  try {
    // Limpieza de posibles bloques de código o texto extra
    const rawClean = raw
      .replace(/```json/gi, "")
      .replace(/```/g, "")
      .trim();
